...
const OPENAI_CALL_CONCURRENCY = 5;
...
function normalizeTitleForSimilarity(title) {
  return String(title || "")
    ...
    .replace(/\bstruck? a deal\b|\bstrikes? a deal\b/g, "deal")
    .replace(/\bstrikes?\b/g, "strike")
    .replace(/\bceasefire\b|\btruce\b/g, "pause")
    .replace(/[^a-z0-9\s]/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}
...
function eventSignature(articleOrText) {
  ...
  if (/(trump).*(extend|pause|deadline|ceasefire).*(iran|deal|attack|energy|strike)|(trump).*(iran|ceasefire|deal).*(extend|pause|deadline)/.test(text)) {
    return "trump-iran-extension";
  }
...
async function classifyDirectionAndScenario(promptText) {
  const raw = await callOpenAiPlainText(
    [
      "You classify news headlines for a macro oil and US Treasury yield dashboard.",
      "Return exactly two tokens separated by a pipe character: DIRECTION|SCENARIO",
      "DIRECTION: ESCALATORY or DE-ESCALATORY. Focus on the action verb and outcome, not topic nouns.",
      "SCENARIO: escalation, contained, or deescalation.",
      "Hard constraints: ESCALATORY maps only to escalation or contained. DE-ESCALATORY maps only to deescalation or contained.",
      "IMPORTANT: 'strike a deal' means negotiate or agree — it is diplomatic language, NOT military action. Classify it as DE-ESCALATORY.",
      "Examples:",
      "  Iran rejects ceasefire → ESCALATORY|escalation",
      "  Talks make progress → DE-ESCALATORY|contained",
      "  Ceasefire signed, Hormuz reopens → DE-ESCALATORY|deescalation",
      "  Trump extends pause of strikes → DE-ESCALATORY|contained",
      "  Marines deployed to Persian Gulf → ESCALATORY|escalation",
      "  Oil falls on diplomatic signals → DE-ESCALATORY|contained",
      "  Trump extends deadline for Iran to strike a deal → DE-ESCALATORY|contained",
      "  Strikes a deal with Iran → DE-ESCALATORY|deescalation",
      "  Iran and US strike a deal on nuclear program → DE-ESCALATORY|deescalation",
      "Return only DIRECTION|SCENARIO. No spaces around the pipe. No other text."
    ].join("\n"),
    promptText
  );
  const pipe = raw.indexOf("|");
  if (pipe === -1) {
    throw new Error(`No pipe in LLM response: ${raw}`);
  }
  const direction = normalizeDirectionLock(raw.slice(0, pipe).trim());
  const scenario = normalizeScenarioFromDirection(raw.slice(pipe + 1).trim(), direction);
  if (!direction || !scenario) {
    throw new Error(`Could not parse direction/scenario from: ${raw}`);
  }
  return { direction, scenario };
}
...
async function classifyHeadline(article, requestMemo) {
  ...
  const runner = (async () => {
    ...
    try {
      // Use RSS description as context — avoids slow article-fetch blocking classification
      const context = String(safeArticle.description || "").trim() || null;
      const promptText = buildClassifierPrompt(title, context);
      const { direction, scenario } = await classifyDirectionAndScenario(promptText);
      result = {
        scenario,
...
async function rankHeadlinesByMarketImpact(candidateArticles) {
  if (!ANTHROPIC_API_KEY || !candidateArticles.length) {
    return null;
  }
  const numbered = candidateArticles.slice(0, 15).map((article, i) => {
    const snippet = article.description ? ` — ${String(article.description).slice(0, 120)}` : "";
    return `[${i}] ${article.title}${snippet}`;
  }).join("\n");
  ...
  try {
    const payload = await openAiLimiter(() => fetchJson("https://api.anthropic.com/v1/messages", {
      ...
      body:JSON.stringify({
        ...
        max_tokens:48,
        system:[
          "You rank news headlines strictly by market-moving potential for US fixed income (Treasury yields, curve shape) and oil (Brent crude, WTI).",
          "Prioritize: Hormuz closure or reopening, oil supply disruption or relief, oil export sanctions, force deployments affecting shipping lanes, ceasefire or deal that reopens flows, inflation repricing from energy shocks, Fed/Treasury market reaction headlines.",
          "Deprioritize: general war commentary, opinion, analysis, Asian/European equity market reactions, gold, bitcoin, equities without a clear rates or oil link.",
          "Return only 5 integers separated by commas — the 0-based indices from the input list, ordered from most to least market-moving for oil and US rates. No explanation, no other text."
        ].join(" "),
    ...
    const indices = text.split(/[\s,]+/)
      .map(Number)
      .filter((n) => Number.isFinite(n) && n >= 0 && n < candidateArticles.length);
...
function analyzeHeadlineContext(articleOrText) {
  const text = articleText(articleOrText).replace(/\bstri(?:ke|kes|kes?|ck)?\s+a\s+deal\b/gi, "reach a deal");
...
async function deriveIntelFromArticles(articles, requestMemo = createClassificationRequestMemo()) {
  ...
  // Build a wider heuristic candidate pool (up to 15) for AI ranking
  let rankingPool = selectTopArticles(headlineBase, 15, headlineFeedPriority, [1, 3, 6, 12, 18, 24, 36, 48]);
  if (rankingPool.length < 5) {
    rankingPool = extendUniqueArticles(rankingPool, cleanPool, 15, headlineFeedPriority);
  }

  // Try AI ranking by market-moving potential for Brent/WTI/Treasury yields
  let headlineArticles = null;
  const aiIndices = await rankHeadlinesByMarketImpact(rankingPool);
  if (aiIndices) {
    headlineArticles = aiIndices.map((i) => rankingPool[i]).filter(Boolean);
  }

  // Heuristic fallback: take top 5 from the ranking pool
  if (!headlineArticles || headlineArticles.length < 3) {
    headlineArticles = rankingPool.slice(0, 5);
  }
  ...
  // LLM-classify only the 5 displayed headlines; heuristic-classify everything else.
  // This ensures all 5 headline articles always get Anthropic classification (5 parallel
  // calls through concurrency-5 limiter) without competing with probability/curve articles.
  const headlineTitleSet = new Set(headlineArticles.map((a) => a.title));
  ...
  const enriched = await Promise.all(classificationPool.map(async (article) => {
    let classification;
    if (headlineTitleSet.has(article.title)) {
      classification = await classifyHeadline(article, requestMemo);
    } else {
      const hCtx = analyzeHeadlineContext(article);
      classification = { scenario:hCtx.scenario, sentiment:hCtx.sentiment, direction:null, classification_status:"heuristic" };
    }
...
async function normalizeIntelPayload(raw, articles, source, requestMemo = createClassificationRequestMemo()) {
  ...
  return {
    ...
    ...derived,
    headlines:derived.headlines.length ? derived.headlines : headlines,
    probabilities,
